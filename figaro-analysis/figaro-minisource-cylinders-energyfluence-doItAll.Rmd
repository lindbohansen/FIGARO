...
### kerma originally: MeV g-1 = 1.60217662 × 10-7 mGy
### kerma new: 10-7 mGy
...
# Grabbing simulated data for smoother points around nanoDots cosm points

cosmsData=tibble(iX=numeric(),iY=numeric(),iZ=numeric(),wkerma=numeric())
cosmsDataNeat=tibble(x=numeric(),y=numeric(),z=numeric(),wkerma=numeric())
for(itr in 1:dim(cosms)[1]){
  ...
  cosmsDataNeat=add_row(cosmsDataNeat,x=cosms[itr,]$x,y=cosms[itr,]$y,z=cosms[itr,]$z,wkerma=as.numeric(ungroup(summarise((filter(data,iX<=cosms[itr,]$x+smoother & iX>=cosms[itr,]$x-smoother,iY<=cosms[itr,]$y+smoother & iY>=cosms[itr,]$y-smoother,iZ==cosms[itr,]$z)),wkerma=mean(wkerma)))))
}

# Grabbing simulated data for smoother points around ssdl points

ssdlData=tibble(iX=numeric(),iY=numeric(),iZ=numeric(),wkerma=numeric())
ssdlDataNeat=tibble(x=numeric(),y=numeric(),z=numeric(),wkerma=numeric())
for(itr in 1:dim(ssdl)[1]){
  print(ssdl[itr,])
  ssdlData=bind_rows(ssdlData,ungroup(summarise(group_by(filter(data,iX<=ssdl[itr,]$x+smoother & iX>=ssdl[itr,]$x-smoother,iY<=ssdl[itr,]$y+smoother & iY>=ssdl[itr,]$y-smoother,iZ==ssdl[itr,]$z),iX,iY,iZ),wkerma=mean(wkerma))))
  ssdlDataNeat=add_row(ssdlDataNeat,x=ssdl[itr,]$x,y=ssdl[itr,]$y,z=ssdl[itr,]$z,wkerma=as.numeric(ungroup(summarise((filter(data,iX<=ssdl[itr,]$x+smoother & iX>=ssdl[itr,]$x-smoother,iY<=ssdl[itr,]$y+smoother & iY>=ssdl[itr,]$y-smoother,iZ==ssdl[itr,]$z)),wkerma=mean(wkerma)))))
}
